...
const cli_context_1 = require("./runtime/cli-context");
 ...
 */
function logConfigSources(logger, metadata) {
...
async function runCli(args = process.argv.slice(2)) {
    ...
    const { config, metadata } = (0, config_1.loadConfigWithMetadata)({
        cli: options.cliConfig,
        envFilePath,
    });
    const event = loadEventPayload(options.payload);
    const context = (0, cli_context_1.createCliContext)({
    ...
    const result = await (0, event_router_1.routeEvent)(event, options.event, client);
    ...
    if (result.outcome.type === 'review') {
        ...
        else if (options.event === 'pull_request_review_comment' &&
    ...
    if (outputPath) {
        (0, node_fs_1.writeFileSync)((0, node_path_1.resolve)(process.cwd(), outputPath), JSON.stringify(output, null, 2), 'utf8');
        logger.info('Wrote CLI output', { filePath: outputPath });
    }
}
function loadEventPayload(payloadPath) {
    try {
        return JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.resolve)(process.cwd(), payloadPath), 'utf8'));
    }
    catch (error) {
        throw Object.assign(new Error(`Failed to read CLI payload from ${payloadPath}: ${error instanceof Error ? error.message : String(error)}`), { cause: error });
    }
}
/**
 ...
 */
function resolveGitHubToken(token, envFilePath) {
    if (token && token.trim() !== '') {
 ...
 */
function getLocalEnvGitHubToken(envFilePath) {
    try {
        return (0, config_1.loadLocalEnv)(process.cwd(), envFilePath).GITHUB_TOKEN?.trim();
    }
    catch {
        return undefined;
    }
...
async function listPullRequests(options) {
    ...
    try {
        const prs = await (0, pull_requests_1.listPullRequests)(client, {
            owner,
            repo,
            state,
            limit,
 ...
 */
async function runPrReview(options) {
    ...
    const prIdentity = await (0, pull_requests_1.fetchPullRequestIdentityByNumber)(client, owner, repo, prNumber);
    if (!prIdentity) {
 ...
 */
function getDefaultOutputPath(owner, repo, prNumber) {
 ...
 */
function resolveOutputPath(options, repoFullName, prNumber) {
...
function printPullRequestList(prs, owner, repo, state, limit) {
