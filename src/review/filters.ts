...
import { hasReviewableLine } from '../diff/normalize-file-diffs';
 ...
 */
export function filterActionableOnly(findings: AIComment[]): AIComment[] {
 ...
 */
export function filterDiffScoped(findings: AIComment[], fileDiffs: FileDiff[]): AIComment[] {
  ...
  return findings.filter((finding) => {
    ...
    return hasReviewableLine(fileDiff, finding.line);
  });
}
 ...
 */
export function deduplicateFindings(findings: AIComment[]): AIComment[] {
  const seen = new Set<string>();
  const result: AIComment[] = [];
 ...
 */
export function filterForInlinePosting(findings: AIComment[]): AIComment[] {
  return findings.filter((finding) => {
 ...
 * @returns Filtered array of actionable findings
 */
export function filterFindings(findings: AIComment[], fileDiffs: FileDiff[]): AIComment[] {
